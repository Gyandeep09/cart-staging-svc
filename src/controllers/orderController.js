...
const listMyOrders = async (req, res) => {
    try {
        ...
        res.status(200).json({ status: 'ok', count: result.rows.length, orders: result.rows });
    } catch (err) {
...
const getOrderById = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ status: 'ok', order: { ...orderResult.rows[0], items: itemsResult.rows } });
    } catch (err) {
...
const cancelOrder = async (req, res) => {
    ...
    try {
        const existing = await pool.query(`SELECT * FROM orders WHERE id = $1 AND user_id = $2`, [id, req.user.id]);
...
// --- Admin routes below. Read-only by design — admins can see every
// order, but cancelling remains a self-service action only, per scope. ---
...
module.exports = { listMyOrders, getOrderById, cancelOrder, listAllOrders, getAnyOrderById };