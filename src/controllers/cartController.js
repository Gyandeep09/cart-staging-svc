...
const addToCart = async (req, res) => {
    ...
    const { entity_type, entity_id, item_name, unit_price, quantity, currency } = req.body;

    if (!entity_type || !entity_id || !item_name || unit_price === undefined) {
        return res.status(400).json({
            status: 'error',
            message: 'entity_type, entity_id, item_name, and unit_price are required.',
        });
    }

    if (unit_price < 0) {
        return res.status(400).json({ status: 'error', message: 'unit_price cannot be negative.' });
    }
    ...
    if (!isValidCurrencyCode(currencyCode)) {
        return res.status(400).json({
            status: 'error',
            message: 'currency must be a valid 3-letter ISO 4217 code (e.g. INR, USD, EUR).',
        });
    }
    ...
    try {
        const result = await pool.query(
            `INSERT INTO cart_items (user_id, entity_type, entity_id, item_name, unit_price, quantity, currency)
             ...
             RETURNING *`,
            [user_id, entity_type, entity_id, item_name, unit_price, qty, currencyCode]
        );
...
const updateCartItem = async (req, res) => {
    ...
    if (quantity === undefined || quantity <= 0) {
        return res.status(400).json({
            status: 'error',
            message: 'quantity is required and must be greater than 0. Use DELETE to remove an item.',
        });
    }
    ...
    try {
        ...
        if (result.rows.length === 0) {
            return res.status(404).json({ status: 'error', message: 'Cart item not found, or does not belong to you.' });
        }
...
const checkout = async (req, res) => {
    ...
    try {
        ...
        const cartResult = await client.query(`SELECT * FROM cart_items WHERE user_id = $1`, [user_id]);
        ...
        const distinctCurrencies = [...new Set(cartResult.rows.map((item) => item.currency))];
        ...
        const orderResult = await client.query(
            `INSERT INTO orders (user_id, status, total, currency) VALUES ($1, 'pending_payment', $2, $3) RETURNING *`,
            [user_id, total.toFixed(2), orderCurrency]
        );
        ...
        for (const item of cartResult.rows) {
            await client.query(
                `INSERT INTO order_items (order_id, entity_type, entity_id, item_name, unit_price, quantity, currency)
                 VALUES ($1, $2, $3, $4, $5, $6, $7)`,
                [order.id, item.entity_type, item.entity_id, item.item_name, item.unit_price, item.quantity, item.currency]
            );
        }
        ...
        res.status(201).json({ status: 'ok', order: { ...order, items: itemsResult.rows } });
    } catch (err) {
        await client.query('ROLLBACK');
        console.error('[Cart-Checkout] Checkout error:', err);
        res.status(500).json({ status: 'error', message: 'Checkout failed. No order was created.' });
    } finally {
        client.release();
    }
};

module.exports = { addToCart, getMyCart, updateCartItem, removeCartItem, checkout };